...
class PpnMasukanImport implements ToCollection, WithEvents
{
    public function registerEvents(): array
    {
        return [
            BeforeImport::class => function (BeforeImport $event) {
                // Hapus data masukan lama
                TaxPpnRecord::where('type', 'masukan')->delete();
            },
        ];
    }
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            try {
                ...
                Log::warning("PpnMasukanImport: Failed to parse row $index - " . $e->getMessage());
            }
        }
    }
}
