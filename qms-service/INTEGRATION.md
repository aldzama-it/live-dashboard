# QMS Company Integration â€” 13 Aug 2026
...
- `qms-service/` â€” service Python QMS yang diambil dari source QMS lama.
- `qms-service/setup-local-data.ps1` â€” helper lokal untuk menyalin data Excel/runtime dari project QMS lama. Data Excel tidak boleh di-commit.
...
- QMS Python service: `127.0.0.1:5002`
...
4. Jalankan `run.ps1` dan cek `http://127.0.0.1:5002/api/health`.
...
`QMSAudit.jsx` membaca `VITE_QMS_SERVICE_URL`. Default lokal adalah `http://127.0.0.1:5002`. Untuk deployment server, URL service QMS harus diisi sesuai alamat service yang benar pada environment build/deploy. Jangan mengandalkan `127.0.0.1:5002` untuk browser user di production.
